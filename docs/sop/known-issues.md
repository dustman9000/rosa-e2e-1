...
## HyperShift CI sector

The integration sector (`hypershift-dev`) has been blocked on a Cluster Service feature-flag interaction (`HypershiftEgressInflightCheckFeature` + `pendingWorkerToggle` causing clusters to get stuck in `validating` and never provision). Staging sector (`hypershift-ci`) works. If HyperShift-specific CI coverage looks stuck in integration, check whether this is still the blocker (epic [ROSAENG-61876](https://redhat.atlassian.net/browse/ROSAENG-61876)) before assuming it's a new problem.

## Staging STS OIDC secret incident (resolved, useful as a diagnostic playbook)

[ROSAENG-67277](https://redhat.atlassian.net/browse/ROSAENG-67277) - staging lease cluster provisioning broke when an unmanaged OIDC config's private-key AWS Secrets Manager secret was permanently deleted (`Failed to get secret value from arn:aws:secretsmanager:...`). Fixed by replacing the config with a Red Hat managed OIDC config (no customer secret to lose). If you see this error class again: check `aws secretsmanager describe-secret` for a soft-delete recovery window first (restorable = easy fix); if truly gone, the whole OIDC config needs replacing, not just the secret.
